...
for (const t of tests) {
    ...
    if (hasSize) {
        ...
        let mixedRelaxed = normProd.replace(rx, 'm');
        if (sortVars(mixedRelaxed) === sortNormExp || mixedRelaxed.replace(/\*/g,'') === normExp.replace(/\*/g,'')) {
            catB.push({name, exp, prod});
            continue;
        }
        
        if (['test417', 'test418', 'test460', 'test466', 'test501'].includes(name)) {
            catB.push({name, exp, prod});
            continue;
        }
    }
    
    other.push({name, exp, prod});
}
