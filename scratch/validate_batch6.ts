...
async function main() {
    ...
    const CPP_FILE = path.join(__dirname, '..', 'validation_corpus_batch6.cpp');
    ...
    // Extract definitions before test1001 to include as prefix
    let prefix = '';
    for (const line of lines) {
        if (line.includes('test1001(')) break;
        prefix += line + '\n';
    }
    ...
    for (const func of functions) {
        ...
        if (testNum < 1001 || testNum > 1200) continue;
